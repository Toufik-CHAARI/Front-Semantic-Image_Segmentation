...
class DataService:
    ...
    def validate_data_directories(self) -> Tuple[bool, List[str]]:
        ...
        if not os.path.exists(self.original_images_dir):
            errors.append(f"Original images directory not found: {self.original_images_dir}")
        else:
    ...
    def get_directory_stats(self) -> dict:
        ...
        if os.path.exists(self.original_images_dir) and os.path.exists(self.ground_truth_dir):
            stats['directories_exist'] = True
            
            # Count files
            try:
                original_files = [f for f in os.listdir(self.original_images_dir) 
