...
class APIService:
    ...
    def predict_mask(
        ...
        try:
            ...
            if file_size > config.MAX_FILE_SIZE_MB * 1024 * 1024:
                raise ValueError(
                    f"Image file is too large. Max size: {config.MAX_FILE_SIZE_MB}MB"
                )
            ...
            raise Exception(
                f"Connection error: Could not connect to API at {self.base_url}"
            )
    ...
    def _parse_response_metadata(
        self, response: requests.Response, file_size: int
    ) -> Dict[str, Any]:
        """
